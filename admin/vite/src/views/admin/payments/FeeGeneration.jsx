...
// @icons
import { IconEye, IconRefresh, IconFilter, IconX } from '@tabler/icons-react';
...
export default function FeeGenerationPage() {
  ...
  const stats = {
    ...
    pendingAmount: fees.reduce((sum, f) => sum + (f.amount - (f.paidAmount || 0)), 0),
  };

  return (
    <ComponentsWrapper title="Fee Generation">
      <Stack spacing={3}>
        {/* Statistics Cards */}
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 6, sm: 4, md: 2.4 }}>
            <Card sx={{ bgcolor: 'info.lighter', color: 'info.main' }}>
              <CardContent>
                <Stack spacing={1}>
                  <Typography variant="h4" fontWeight={600}>{formatPrice(stats.pendingAmount)}</Typography>
        ...
        <PresentationCard>
          <Stack spacing={2}>
            <Typography variant="h6">All Generated Fees</Typography>
            {loading ? (
                    ...
                    {filteredFees.map((fee) => {
                      ...
                      return (
                        <TableRow key={fee._id} hover>
                          ...
                          <TableCell>
                            <Typography variant="body2" color={pendingAmount > 0 ? 'error.main' : 'success.main'} fontWeight={500}>
