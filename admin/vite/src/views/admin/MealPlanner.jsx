...
export default function AdminMealPlanner() {
  ...
  const [reviewData, setReviewData] = useState({ status: 'pending', reviewNotes: '', implementDate: '' });
  ...
  }, [selectedMealSlot, selectedStatus]);
  ...
  const fetchSuggestions = async () => {
    try {
      ...
      const data = await adminService.getMealSuggestions(params);
  ...
  const fetchPreferences = async () => {
    try {
      setLoading(true);
      const data = await adminService.getMealSuggestionPreferences();
      setPreferences(data);
    } catch (err) {
      console.error('Failed to fetch preferences:', err);
      enqueueSnackbar(err.response?.data?.message || 'Failed to load preferences', { variant: 'error' });
      setPreferences(null);
    } finally {
      setLoading(false);
    }
  ...
  const fetchTodayMeal = async () => {
    try {
      ...
      const meals = await adminService.getDailyMeals({ startDate: today });
      if (meals && meals.length > 0) {
        setTodayMeal(meals[0]);
      } else {
        // Use default for today
        const dayIndex = new Date().getDay() - 1; // Monday = 0
  ...
  const fetchWeeklyMeals = async () => {
    try {
      ...
      const meals = await adminService.getDailyMeals({
        startDate: monday.toISOString().split('T')[0],
        endDate: sunday.toISOString().split('T')[0]
      });

      // Create a map of fetched meals by day
      const mealsMap = {};
      meals.forEach(meal => {
      ...
      const weeklyMeals = defaultWeeklyMenu.map(day => {
        if (mealsMap[day.day]) {
          return {
            day: day.day,
            breakfast: mealsMap[day.day].breakfast,
            lunch: mealsMap[day.day].lunch,
            dinner: mealsMap[day.day].dinner,
            fromSuggestion: mealsMap[day.day].fromSuggestion,
            _id: mealsMap[day.day]._id
          };
        }
        return day;
      });
  ...
  const handleOpenReview = (suggestion) => {
    ...
    setReviewData({
      ...
      implementDate: suggestion.implementedForDate 
        ? new Date(suggestion.implementedForDate).toISOString().split('T')[0] 
  ...
  const handleCloseReview = () => {
    ...
    setReviewData({ status: 'pending', reviewNotes: '', implementDate: '' });
  };

  const handleUpdateStatus = async () => {
    try {
      await adminService.updateMealSuggestionStatus(reviewDialog.suggestion._id, reviewData);
      enqueueSnackbar('Suggestion status updated successfully', { variant: 'success' });
      handleCloseReview();
      fetchSuggestions();
      fetchPreferences();
      // Refresh meals if implemented
      if (reviewData.status === 'implemented') {
        if (tab === 'today') {
          fetchTodayMeal();
        } else if (tab === 'week') {
          fetchWeeklyMeals();
        ...
        <Stack spacing={3}>
          ...
          <TabPanel value={tab} index="today">
            <Stack spacing={2}>
              <Typography variant="h6">
                Today's Menu - {new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
              </Typography>
              {loading ? (
                ...
                <Grid container spacing={2}>
                  {['Breakfast', 'Lunch', 'Dinner'].map((meal) => {
                    ...
                    const mealValue = todayMeal?.[mealKey] || 'Menu not available';
                    const fromSuggestion = todayMeal?.fromSuggestion?.[mealKey];
                    return (
                      <Grid key={meal} size={{ xs: 12, md: 4 }}>
                        <Card variant="outlined" sx={{ 
                          bgcolor: fromSuggestion?.suggestionId ? 'success.lighter' : 'background.paper',
                          borderColor: fromSuggestion?.suggestionId ? 'success.main' : 'divider'
                        }}>
                          <CardContent>
                            <Stack spacing={1}>
                              ...
                              {fromSuggestion?.suggestionId && (
                                <Chip 
                                  label={`Suggested by ${fromSuggestion.studentName}`}
                                  size="small"
                                  color="success"
                                  sx={{ mt: 1 }}
                                />
                    ...
                    <CardContent>
                      ...
                      <Stack spacing={1}>
                        <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                          ...
                          {day.fromSuggestion?.breakfast?.suggestionId && (
                            <Chip 
                              label={`by ${day.fromSuggestion.breakfast.studentName}`}
                              size="small"
                              color="success"
                            />
                          )}
                        ...
                        <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                          ...
                          {day.fromSuggestion?.lunch?.suggestionId && (
                            <Chip 
                              label={`by ${day.fromSuggestion.lunch.studentName}`}
                              size="small"
                              color="success"
                            />
                          )}
                        ...
                        <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                          ...
                          {day.fromSuggestion?.dinner?.suggestionId && (
                            <Chip 
                              label={`by ${day.fromSuggestion.dinner.studentName}`}
                              size="small"
                              color="success"
                            />
                          )}
                    ...
                    <Card key={suggestion._id} variant="outlined">
                      <CardContent>
                        <Stack spacing={2}>
                          ...
                          {(suggestion.student || suggestion.studentIdentity) && (
                            <Stack direction="row" spacing={2} flexWrap="wrap">
                              <Typography variant="caption" color="text.secondary">
                                <strong>Student:</strong> {(suggestion.student?.name || suggestion.studentIdentity?.name || 'Unknown')} ({(suggestion.student?.studentId || suggestion.studentIdentity?.admissionNumber || suggestion.studentIdentity?.studentId || 'N/A')})
                              </Typography>
                              {(suggestion.student?.course || suggestion.studentIdentity?.course) && (
                                <Typography variant="caption" color="text.secondary">
                                  <strong>Course:</strong> {suggestion.student?.course || suggestion.studentIdentity?.course}
                                </Typography>
                              )}
                          ...
                          <Stack spacing={0.5}>
                            ...
                            {suggestion.implementedAt && (
                              <Typography variant="caption" color="success.main" fontWeight={600}>
                                ✓ Implemented on: {new Date(suggestion.implementedAt).toLocaleDateString('en-US', { 
                                  weekday: 'long', 
                                  year: 'numeric', 
                                  month: 'long', 
                                  day: 'numeric' 
                                })}
                                {suggestion.implementedForDate && ` (Menu Date: ${new Date(suggestion.implementedForDate).toLocaleDateString('en-US', { 
                                  weekday: 'long', 
                                  year: 'numeric', 
                                  month: 'long', 
                                  day: 'numeric' 
          ...
          <TabPanel value={tab} index="preferences">
            {loading ? (
              ...
              <Alert severity="info">Loading preference data...</Alert>
            ) : (
              <Stack spacing={3}>
                  ...
                  <CardContent>
                    ...
                    {!preferences.preferences || preferences.preferences.length === 0 ? (
                      <Alert severity="info" sx={{ mt: 2 }}>
                        No suggestions available. Students can submit meal suggestions which will appear here.
                      </Alert>
                    ) : (
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            {reviewDialog.suggestion && (
              <>
                ...
                {(reviewDialog.suggestion.student || reviewDialog.suggestion.studentIdentity) && (
                  <Typography variant="body2" color="text.secondary">
                    <strong>From:</strong> {(reviewDialog.suggestion.student?.name || reviewDialog.suggestion.studentIdentity?.name || 'Unknown')} ({(reviewDialog.suggestion.student?.studentId || reviewDialog.suggestion.studentIdentity?.admissionNumber || reviewDialog.suggestion.studentIdentity?.studentId || 'N/A')})
                  </Typography>
                )}
              </>
            )}
            ...
            {reviewData.status === 'implemented' && (
              <TextField
                fullWidth
                type="date"
                label="Implementation Date (Menu Date)"
                value={reviewData.implementDate}
                onChange={(e) => setReviewData({ ...reviewData, implementDate: e.target.value })}
                InputLabelProps={{ shrink: true }}
                helperText="Select the date when this meal should be added to the menu"
              />
            )}
            <TextField
