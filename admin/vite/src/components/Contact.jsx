...
export default function Contact({
  ...
  // Use validationRules if provided, otherwise fall back to contactSchema
  const rules = validationRules || contactSchema;
  
  // Create a key based on dialCode to force Controller remount when country changes
  // This ensures validation rules are updated when country changes
  const controllerKey = useMemo(() => {
    return `contact-${dialCode || '+1'}-${validationRules ? 'custom' : 'default'}`;
  }, [dialCode, validationRules]);
